...
def take_current_quantity(elem):
...
def get_best_selling_product():
	try:
		...
		response = frappe.get_list("Item", fields="*", filters={"is_best_selling": 1, "disabled": 0, "hide": 0}, order_by="item_name ASC", limit_page_length=8)
		for row in response:
...
def get_order(page=0, query=''):
	try:
		user_id = get_user_id_by_session()
		response = frappe.get_list("Sales Order", fields="*", filters={"customer": user_id, "name": ("LIKE", "%{}%".format(query))}, limit_page_length=LIMIT_PAGE, limit_start=page)
		for res in response:
...
def get_order_detail(order):
	try:
		user_id = get_user_id_by_session()
		response = frappe.get_list("Sales Order", fields="name, status, rounded_total, discount_amount", filters={"name": order, "customer": user_id})
		response[0]["item_list"] = frappe.get_list("Sales Order Item", fields="image, item_name, rate, qty, amount", filters={"parent": order})
...
def set_account():
	try:
		...
		uploaded = upload("Customer",req.name,1)
		userDoc = frappe.get_doc("Customer", req.name)
		userDoc.profile_picture = uploaded['file_url']
