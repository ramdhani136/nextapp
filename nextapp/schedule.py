...
import json
from nextapp.base import normalize_firebase_string
...
def update_sales_item():
	offset_date = datetime.now() - timedelta(days=90)  
	print("SELECT COUNT(*) AS total_sales FROM `tabSales Invoice Item` sii WHERE (SELECT status FROM `tabSales Invoice` WHERE name = sii.parent) = 'Paid' AND sii.creation >= '{}' GROUP BY sii.item_codes".format(offset_date))
	total_sales = frappe.db.sql("SELECT COUNT(*) AS total_sales FROM `tabSales Invoice Item` sii WHERE (SELECT status FROM `tabSales Invoice` WHERE name = sii.parent) = 'Paid' AND sii.creation >= '{}' GROUP BY sii.item_codes".format(offset_date), as_dict=True)[0]["total_sales"]
